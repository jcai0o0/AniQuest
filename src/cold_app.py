...
from src.reranker_cold import rank_anime_by_query
import pandas as pd
...
os.environ["OMP_NUM_THREADS"] = "1"  # Limit LightGBM to a single thread
...
anime_data_path = Path(Path(__file__).parent, "data/", "final_anime_list_cleaned_3rd_normal.csv")
# Load anime data
anime_data = pd.read_csv(anime_data_path)

# Load LightGBM model
model_path = Path(Path(__file__).parent, "data/", "lambda_mart_best_model_optuna.bin")
...
def integration_cold(query):
    anime_name_list = query_chroma(query=query, anime_count=100)
    anime_name_list = rank_anime_by_query(query=query, anime_titles=anime_name_list , anime_data=anime_data, model=model, sentence_model=sentence_model)[:4]
...
with gr.Blocks(theme=gr.themes.Soft(), css=css) as demomo:
    ...
    gr.HTML("""
            <div style="color: red; margin-bottom: 1em; text-align: center; padding: 10px; background: rgba(255,0,0,0.1); border-radius: 8px;">
                ⚠️ Welcome to this recommendation system ⚠️
            </div>
        """)
...
if __name__ == "__main__":
    # print(integration_cold(query="I want something like Demon Slayer, but with more romance and produced by Kyoto Animation"))
    # cold user
    demomo.launch(server_name="0.0.0.0", server_port=7777)